...
class MatchGame extends Component {
  ...
  startTimer = () => {
    ...
    if (counter !== 0) {
      ...
      clearInterval(this.timerID)
      this.setState({isGameInProgess: false})
    }
  }
  ...
  generateRandomImage = () => {
    ...
    this.setState({activeImg: imagesList[index].imageUrl})
  } */
  ...
  onClickMatchImage = id => {
    ...
    const index = Math.ceil(Math.random() * imagesList.length) - 1
    this.setState({activeImg: imagesList[index].imageUrl})
    const i = imagesList.findIndex(img => img.imageUrl === activeImg)
  ...
  renderScoreCard = () => {
    ...
    return (
      <>
        <div className="scorecard-contianer">
          <img
  ...
  renderGame = () => {
    const {activeImg, activeTabId} = this.state
    ...
    return (
      <>
        <div className="match-img-container">
          <img className="match-img" src={activeImg} alt="match" />
        </div>
          ...
          <ul className="tab-ul-container">
            {tabsList.map(eachTab => (
              <TabItem
                key={eachTab.tabId}
                tabDetails={eachTab}
                activeTabId={eachTab.tabId === activeTabId}
          ...
          <ul className="thumbnail-ul-container">
            {filteredList.map(eachImage => (
              <Thumbnail
                ...
                onClickImage={this.onClickMatchImage}
              />
            ))}
          </ul>
        </div>
  ...
  render() {
    const {score, counter, isGameInProgess} = this.state
    ...
    return (
      <div className="bg-container">
        ...
        {isGameInProgess ? this.renderGame() : this.renderScoreCard()}
      </div>
    )
  }
}
